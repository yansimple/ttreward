...
setInterval(() => {
  $("#online").textContent = (5180 + Math.floor(Math.random() * 160)).toLocaleString("ru-RU");
}, 1600);
...
function loadVideo() {
  ...
  v.onerror = () => {
    ...
    $("#lockSub").textContent = "Place " + videoFiles[videoIndex] + " next to index.html";
  };
...
function chooseReaction(btn) {
  ...
  document.querySelectorAll(".reaction").forEach((x) => x.classList.remove("active"));
...
function fillLists() {
  const rankHtml = ranks
    .map(
      (r) => `
        <div class="rank-row">
            <div class="rank-num">${r[0]}</div>

            <div class="rank-person">
                <div class="rank-avatar">${r[3]}</div>

                <div>
                    <div class="rank-name">${r[1]}</div>
    ...
    .map(
      (w) => `
        <div class="row">
            <div>
                <b>${w[0]}</b><br>
                <span class="muted">${w[1]}</span>
            </div>

            <div class="money cyan">${w[2]}</div>
        </div>
    `,
    )
    .join("");
      ...
      .map(
        (r) => `
            <div class="review-card">
                <b>${r[0]}</b>
                <span class="muted" style="float:right">${r[1]}</span><br>
                ⭐⭐⭐⭐⭐
                <p class="muted" style="margin:8px 0 0">${r[2]}</p>
            </div>
        `,
      )
      .join("") +
...
(function () {
  ...
  function finishVerification() {
    ...
    if (h) {
      h.textContent = "✅ PROFILE VERIFIED";
    }
  ...
  function startVerification() {
    ...
    if (h) {
      h.textContent = "♢ PROFILE VERIFICATION";
    }
  ...
  function showPopup() {
    ...
    box.innerHTML =
      '<div class="payout-popup-critical__avatar"><span>T</span></div>' +
      "<div>" +
      '<div class="payout-popup-critical__name">' +
      name +
      "</div>" +
      '<div class="payout-popup-critical__city">' +
      pick(cities) +
      "</div>" +
      "</div>" +
      '<div class="payout-popup-critical__amount">+$' +
      amount() +
      "</div>";
  ...
  function restartPopups() {
    ...
    function scheduleNextPopup() {
      var randomDelay = Math.floor(Math.random() * 3000) + 5000;
  ...
  setInterval(function () {
    if (activePageId() === "selected") {
      ...
      if (btn && verify && verify.classList.contains("is-done") && btn.disabled) {
        finishVerification();
      }
    }
  }, 500);
